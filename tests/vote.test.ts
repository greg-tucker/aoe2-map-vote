...
describe("map voting",()=>{
  it("contains the complete built-in Standard Random Map catalogue",()=>{expect(maps.length).toBeGreaterThanOrEqual(100);expect(new Set(maps.map(m=>m.id)).size).toBe(maps.length)});
